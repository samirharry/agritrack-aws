...
echo Executing command: aws managedblockchain create-network --region $REGION \
    ...
    --name "${NETWORKNAME}" \
    --description "Agritrack Fabric network"
    ...
    --member-configuration "Name=\"${MEMBERNAME}\",Description=\"Agritrack Fabric member\",FrameworkConfiguration={Fabric={AdminUsername=${ADMINUSER},AdminPassword=${ADMINPWD}}}"
